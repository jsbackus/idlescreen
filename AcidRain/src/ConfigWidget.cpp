...
void ConfigWidget::importClicked(bool checked) {
  ...
  for(int i = 0; i<bkgnds.size(); i++) {
	...
	IndexedPaletteProfile tmpPal = tmpMgr->getPaletteProfile(tmpBkgnd->getPaletteName(j));
	_manager->addPalette(tmpPal);
      }
...
void ConfigWidget::exportClicked(bool checked) {
  ...
  for(int i = 0; i<bkgnds.size(); i++) {
    ...
    if(tmpBkgnd == NULL)
      return;
    for(int j=0; j<tmpBkgnd->getNumStyles();j++) {
      IndexedPaletteProfile tmpPal = _manager->getPaletteProfile(tmpBkgnd->getPaletteName(j));
      tmpMgr->addPalette(tmpPal);
    }
